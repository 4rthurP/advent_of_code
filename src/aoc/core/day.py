...
class AOCDay:
    ...
    def load_puzzles(self):
        ...
        for n in range(1, self.n_parts + 1):
            ...
            if not puzzle.has_input:
                self.error(
                    f"Part {n} of day {self.day} is missing an input ({puzzle.input_path})."
                )
                return
        ...
        for puzzle in self.puzzles:
            ...
            match puzzle.state:
                ...
                case PuzzleState.SOLVED:
                    console.print(
                        f"[green]Correct ! {puzzle.given_answer} was the right answer for part {puzzle.part} ![/green]"
                    )
            ...
            console.print(f"[dim]Puzzle solved in {puzzle.timer:.4f}s[/dim]")
        return
    ...
    def _load_class(self, part: str) -> tuple[AOCPuzzle, True] | tuple[str, False]:
        ...
        try:
            ...
            return (
                f"Class for Day {self.day} part {part} of {self.year} does not exist",
                False,
            )
